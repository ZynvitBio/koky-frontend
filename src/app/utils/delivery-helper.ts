...
export function getDynamicAnnouncementText(): string {
  if (typeof window === 'undefined') {
    return 'Pedidos antes de las 8:00 PM se entregan mañana en Bogotá'; // Fallback para SSR
  }
  ...
  while (true) {
    if (!isWeekendOrHoliday(deliveryDate)) {
      ...
      cutoffDate.setHours(20, 0, 0, 0); // Corte a las 8:00 PM del día anterior
  ...
  if (isToday) {
    ...
    if (diffDays === 1) {
      return `Pedidos antes de las 8:00 PM se entregan mañana (${deliveryDayName}) en Bogotá`;
    } else {
      return `Pedidos antes de las 8:00 PM se entregan el ${deliveryDayName} en Bogotá`;
    }
  } else if (isTomorrow) {
    return `Pedidos antes de las 8:00 PM de mañana se entregan el ${deliveryDayName} en Bogotá`;
  } else {
    return `Pedidos antes del ${cutoffDayName} a las 8:00 PM se entregan el ${deliveryDayName} en Bogotá`;
  }
}
